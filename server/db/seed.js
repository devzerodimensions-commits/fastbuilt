...
async function seed() {
  ...
  for (const p of PROJECTS) {
    await pool.query(
      ...
      [p.slug, p.name, p.category, p.location, p.client, p.year, p.status, p.contract_type, p.team, p.summary, p.image, order++]
    )
  }
  console.log(`✓ seeded ${PROJECTS.length} projects`)

  await pool.query('TRUNCATE workers RESTART IDENTITY')
  order = 0
  for (const w of WORKERS) {
    await pool.query('INSERT INTO workers (name, role, image, sort_order) VALUES ($1,$2,$3,$4)', [w.name, w.role, w.image, order++])
  }
  console.log(`✓ seeded ${WORKERS.length} workers`)

  await pool.query('TRUNCATE team RESTART IDENTITY')
  order = 0
  for (const t of TEAM) {
    await pool.query('INSERT INTO team (name, role, image, sort_order) VALUES ($1,$2,$3,$4)', [t.name, t.role, t.image, order++])
  }
